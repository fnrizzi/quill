...
int main()
{
  ...
  std::string ss{"test"};
  LOG_INFO(quill::get_logger(), "This is a log info example {} {} {}", 5, ss, 5.5);

  for (int i = 0; i < 2; ++i)
  {
    LOG_ERROR(quill::get_logger(), "This is a log info example {}", "testzz");
  }
  ...
  //    // Default pattern : "%(ascii_time) [%(thread)] %(filename):%(lineno) %(level_name)
  //    %(logger_name) - %(message)" quill::Logger* logger = quill::get_logger();
  //
  ...
  //            "system_error");
  //
  //  LOG_WARNING(default_logger, "Support for int: {0:d};  hex: {0:x};  oct: {0:o}; bin: {0:b}",
  //  42); LOG_CRITICAL(default_logger, "Easy padding in numbers like {:08d}", 12);
  //
